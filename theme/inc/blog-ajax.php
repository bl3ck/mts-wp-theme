...
// Enqueue scripts for blog archive
function blog_archive_scripts()
{
    if (is_home() || is_archive() || is_page_template('page-blog.php')) {
        // Enqueue the blog archive JavaScript file
        wp_enqueue_script(
            'blog-archive', 
        ...
        wp_localize_script('blog-archive', 'blogArchive', array(
            'ajaxUrl' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('blog_posts_nonce')
        ));
    }
}
add_action('wp_enqueue_scripts', 'blog_archive_scripts');
